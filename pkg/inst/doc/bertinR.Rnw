...
The \R{}-implementation can be downloaded as a package \ircode{bertin} from \url{http://bertin.r-forge.r-project.org/}.  \cite{seriation:Falguerolles:1997} is included as bertin.pdf in the documentation section of the package.
...
col=ifelse(Hotel[19,]>mean(as.matrix(Hotel[19,])),"black","white")
 bertinrect(Hotel[19,], main="", col=col, pars = list(mar=c(0.5,0.3,2,8)))
# par(oldpar)
...
bertinrect(Hotel2, main="", col=col,  pars = list(mar=c(0.2,0.2,2,8)))
@
...
bertinrect(Hotel2[rowperm,], main="", 
	col=col[rowperm,],  pars = list(mar=c(0.2,0.2,2,8)))
@
...
bertinrect(t(USJudgeRatings),col=t(col), sepwd=0.1,  pars = list(mar=c(0.05,0.05,10,4)), main="")
...
<<fig=TRUE, include = TRUE, echo=TRUE, label=judgeperm1rng,width=12, height=6>>=
...
image(zscores[rowperm,], main="", col=gray((0:256)/256), useRaster=FALSE)
#par(oldpar)
@
<<fig=TRUE, include = FALSE, echo=FALSE, width=6, height=6, label=hotelpermimage2>>=
image(zscores[rowperm,], main="", col=gray((0:3)/3), useRaster=FALSE)
@
%
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.48\linewidth]{"bertinR-hotelpermimage1"}\includegraphics[width=0.48\linewidth]{"bertinR-hotelpermimage2"}
...
Colour may need some experiments to find an informative choice. To allow easy experiments, we use a two step procedure. Based on the original data or the score, we derive a colour index. From an abstract point of view, this is just another score with values in $1, \ldots nrColours$. Colours are provided as a colour palette, and the index is used to select the colour to apply. 
...
There is an abundant literature on chosing colour palettes for the visual display of quantitative information. Please read.

Be aware that not all people perceive colour the same way. In perticular, if you are using red and green in you coulour palette, about 6\% of the male popultion will have difficulties.

Do trivial tests. If you have a smat colour scale: give a sample to some frinds and ask tehm tio tell which shoes higher and which shows lower values.
...
\vspace{\stretch{1}}
\pagebreak
<<fig=TRUE, echo=TRUE, width=6, height=3, label= palgreencolors>>=
...
\vspace{\stretch{1}}
\pagebreak
<<fig=TRUE, echo=TRUE, width=6, height=3, label= palgreenred2colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(1, 1, 4, 1) + 0.1)
...
We provide prototypes for the display of Bertin matrices. To simplify the implementation of extensions, we chose a coordinate system that allots unit square to each matrix cell. If we want to add separator lines to structure our data, we have to reserve some space \ircode{sepwd}, measured in user space. 

The convention for a matrix\ircode{x} the data\ircode{x[i, j]}, the display is a unit square with bottom left corner at \ircode{(i, j)}.
Coordinates follow matrix conventions,  that is the $y$ axis is pointing downwards, and the top right corner is at \ircode{(i+1, j-1)}.

Additional graphical elements can be used immediately, using this coordinat system
\ircode{image} uses the center of a cell as an anchor point. To use \ircode{image} to generate an overlay, change the user coordinate system temporarily to \ircode{par("usr")-0.5}.

We return the relevant graphical parameters as an invisible result in the basic  functions \ircode{imagem()} and \ircode{bertinrect}. This allows to generate a sequence of graphics with consistent layout.
