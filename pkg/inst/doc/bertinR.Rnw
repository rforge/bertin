...
plot.bertin(Hotel2, roworder=rowperm,
	...
	showpalette=FALSE)
title(sub="Variables rearranged", line=1,cex.sub=1.8)
...
The calling structure for all examples in this section is the same.
...
In RGB space, each palette has natural variants corresponding to gamma corrections. Standardised on $[0, 1]$, gamma correction replaces an intensity $i$ by $i^\gamma$. This is in analogy to the channel gamma correction used technically.

Note: it is up to discussione, whether $\gamma$ should modify the input intensity, or the channel response. This may change.

<<fig=TRUE, echo=TRUE,  width=4, height=1.9,  label= palgredgammacolors>>=
oldpar <- par(mfrow=c(3,1), mar=c(2, 1, 2, 1) + 0.1)
colramp(bertin:::red.colors(100, igamma=0.5))
colramp(bertin:::red.colors(100))
colramp(bertin:::red.colors(100, igamma=1.5))
par(oldpar)
@
...
oldpar <- par(mfrow=c(3,1), mar=c(2, 1, 2, 1) + 0.1)
colramp(bertin:::wred.colors(100, igamma=0.5))
colramp(bertin:::wred.colors(100))
colramp(bertin:::wred.colors(100, igamma=1.5))
par(oldpar)
@
...
<<fig=TRUE, echo=FALSE,  width=4, height=2,  label= palblueredcolors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
<<fig=TRUE, echo=FALSE,  width=4, height=2,  label= palbluered2colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
%\pagebreak
<<fig=TRUE, echo=FALSE,  width=4, height=2,  label= palbluered4colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
\clearpage
<<fig=TRUE, echo=FALSE, width=4, height=2, label= palgreenred2colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
%\pagebreak
<<fig=TRUE, echo=FALSE, width=4, height=2, label= palgreenred4colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
<<fig=TRUE, echo=FALSE, width=4, height=2, label= paltailcolors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
\begin{center}
<<fig=TRUE,  width=6, height=8, label= brewercolors>>=
