...
<<fig=TRUE, include = FALSE, echo=FALSE, results=hide, label=hotel19, width=6, height=2 >>=
...
Variables need not enter at their face value; they can be transformed, or derived variable can e added. In the case of the hotel data, this has already done in the original data set. For example, the guests have been classified in tourists and business, and both sum up to 100\%.  If we want, we can remove this redundant information. This may clean up the picture. But it may hide information. For example, >tourists are ``anti-cyclic'' to the hotel occupation and just fill the gaps. Removing this variable because it is $1-business$ would hide this point.
...
\caption{Display of a data matrix: \textbf{USJudgeRatings data}. Lawyers' ratings of state judges in the US Superior Court.}
\label{fig:judgeorig}
\end{center}
\end{figure}
...
title( main="US Judge Ratings", line=5)
title( sub="observations and variables sorted by mean", line=1)
...
title( main="US Judge Ratings", line=5)
title( sub="observations and variables sorted by mean. CONT removed.", line=1)
...
The highlight features of the  display give a clear picture of a rather homogeneous upper and homogeneous lower group of judges. All criteria seem to say the same in these groups. For a small intermediate group of cases, the variables seem to fall into two or three groups, and it is only for this small group that a closer look at the variables and their meaning may be necessary. 
 If you want to model the rating of the worthiness of retention, say, as a function ot the other ratings, it is only here that a refined model might be necessary, whereas in the case most judges a rough model would do.
 
 For this data set, the low resolution of the variables may be a problem. No details are given in the documentation, but  presumably the rating has been taken on a $1, \ldots, 10$ scale, and in all variables the median is higher than $7$. Changing the scale may give a better contrast and improve the resolution (see \reffig{fig:judgeperm1rng}).  Here we used the rescaled variables to define the height of the bars. For highlighting by colour (only black/white here) we used the data in the original scale. Both display attributes, height and colour, are independent and we are free to encode different aspects of the data. In  \reffig{fig:judgeperm1rng} we sorted the cases by the value of \ircode{RTEN}. This lets us view the data from a different point of view.
 
Again we have a clear upper and lower group, with consistency over all variables, and a small intermediate group which needs closer inspection. Judge G.A. Saden stands out. By most criteria, he qualifies for a higher ranking, but demeanor and judicial integrity go along with a low scored worthy of retention.
...
title(sub="Variable sorted by correlation to Occupation", cex.sub=0.6, line=1)
par(oldpar)
@
...
This score just rescales to $[0, 1]$. On contrast to ranks, it preserves quantitative proportions. See \reffig{fig:judgeperm1rng}.
...
title( main="US Judge Ratings", line=5)
title( sub="Cases sorted by RTEN, variables sorted by mean. Rescaled for range. CONT removed.", 
	line=1, cex.sub=0.6)
@
...
Colour is one of the simplest qualities, and barely understood. One basis of colour is light, and if we follow todays physics this is best described as electromagnetic wave with energy distributed over a spectum of wavelenghts. So the mathematiical home is some Hilbert space.

We do not know how light translate into perceived colour, but we know some of the steps, and we have some artefacts that may help us to understand the process. Today, the most helpful concept seems to be that of a colour space - a finite dimensional space mapped into the true space of colours. 
...
Of course there is a huge discrepancy between the full Hilbert space of lights, and three dimensional RGB space.  The gap is bridged by perception. Different spectral distributions may give the same perceived coulours. By mixing only few colours, you can generate the impression to nearly cover a spectrum. Colours thus matched perceptually are called metamers. 
...
What they tell us in school is that all colours can be mixed from three basic colours. No, note quite. Not even for monochrome light. For many frequency, the impression can be generated by an RGB combination. For several colours in the green range, it is necessary to reduce the colour by a complement before it is accessible in RGB space. See \url{<http://www.statlab.uni-heidelberg.de/data/color/>}. However the mapping  from monochromous coulor space to RGB space is fairly understood and allows us  to generate (within limits) the colour of any frequency if we can control RGB values.

Technology requires differin spaces. While for displays we originally had three colours encoded in RGB, for printing the readily available dyes ware cyan, magenta and yellow, defining a CMY space. Fortunately these dyes could be calibratetd to be roughly complimentary to RGB, so we think of a colour cube with pure R, G, B on three corners, C, M, Y on the opposite corners and an easy translation from RGB to CMY coordinates. Today, quality colour printing is starting with six dyes, and a more complex colour space is standard.
...
<<fig=TRUE, echo=TRUE, width=4, height=2, label= palgreencolors>>=
...
\vspace{\stretch{1}}
%\pagebreak
<<fig=TRUE, echo=TRUE,  width=4, height=2,  label= palgredcolors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
\vspace{\stretch{1}}
%\pagebreak
<<fig=TRUE, echo=TRUE,  width=4, height=2, label= palbluecolors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
<<fig=TRUE, echo=TRUE,  width=4, height=2,  label= palblueyellowcolors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
<<fig=TRUE, echo=TRUE,  width=4, height=2,  label= palblueyellow2colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
\vspace{\stretch{1}}
%\pagebreak
<<fig=TRUE, echo=TRUE,  width=4, height=2,  label= palblueyellow4colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
\vspace{\stretch{1}}
%\pagebreak
<<fig=TRUE, echo=TRUE,  width=4, height=2,  label= palbluered2colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
\vspace{\stretch{1}}
%\pagebreak
<<fig=TRUE, echo=TRUE,  width=4, height=2,  label= palbluered4colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
\vspace{\stretch{1}}
%\pagebreak
<<fig=TRUE, echo=TRUE, width=4, height=2, label= palgreenred2colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
\vspace{\stretch{1}}
%\pagebreak
<<fig=TRUE, echo=TRUE, width=4, height=2, label= palgreenred4colors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
<<fig=TRUE, echo=TRUE, width=4, height=2, label= paltailcolors>>=
oldpar <- par(mfrow=c(2,1), mar=c(2, 1, 2, 1) + 0.1)
...
We provide prototypes for the display of Bertin matrices. To simplify the implementation of extensions, we chose a coordinate system that allots a unit square to each matrix cell. If we want to add separator lines to structure our data, we have to reserve some space \ircode{sepwd}, measured in user space. 
...
\end{figure}
%
The device space may be alloted to several figures, leading to a hierachy of coordinate systems and displays (\reffig{fig:baseregionsnfig}).
...
<<fig=FALSE, eps= FALSE, echo=FALSE, include=FALSE>>=
dev.off()
@
%
\begin{figure}[htbp]
...
\caption{\textbf{Borderline data}. Tension level, all observations, by time. Spot the cases in the control group (Id index > 50) who have tension levels that rather fit into the patient group.}\label{fig:timemapseqtopo}\label{fig:timemapseqtopoinv}
\end{center}
\end{figure}
...
This display is used in the quality control of the process. The aim is to decide about the quality of a chip, or of a production protocol. The identification of differentially expressed genes, and ultimately the identification of tumor genes is a different question. To interpret the plot, you need some information about the null behaviour. The probes are not spotted at random, but slected from  gene libraries. In this case two libraries have been used, one of kidney related genes, and one of presumably tumour related genes. You expect differences between the upper half and lower half, reflecting these libraries. Not all spots have been used, and you expect bands of unused spots. But other inhomogeneities are informative, for example the accumulation of bad spots on this chip on the sides, which here comes from an asymmetrie in handling the cubation cells for the chip - a production problem which can be overcome, once you have identified it.
...
testplot <- function(z, main=deparse(substitute(z)))
{
	...
	title(sub="bertinrect", line=1, cex.sub=0.6)

par(mfg=c(1,2)) # fix for stray display allocation
	imagem(z, main= main)
	title(sub="imagem", line=1, cex.sub=0.6)
...
par(mfg=c(2,1)) # fix for stray display allocation -- this is very very bad
	title(sub="plot.bertin", line=0, cex.sub=0.6)
par(mfg=c(2,2)) # fix for stray display allocation
	image.bertin(z, main= main)
	title(sub="image.bertin", line=0,  cex.sub=0.6)
		par(oldpar)
}
@
%testplot <- function(z, main=deparse(substitute(z)))
