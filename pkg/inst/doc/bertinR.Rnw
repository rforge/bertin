...
We return the relevant graphical parameters as an invisible result in the basic  functions \ircode{imagem()} and \ircode{bertinrect}. The \ircode{usr} parameters are set to have $(1,1)$ as the bottom left corner, and \ircode{x[i, j]} has a unit square figure area with bottom left coordinates at $nrow(x) - i +1, j$, with drawing coordinates following the usual plot conventions, that is the $y$ axis is pointing upwards.

%The drawing area for cell \ircode{x[i, j]} is a unit square with bottom left coordinates \ircode{ (nrrows-i+1)*} 
%\ircode{(1+2*sepwd) -} \ircode{sepwd*} \ircode{(1+} \ircode{2*sepwd)} \ircode{- sepwd,} \ircode{j* (1+2*sepwd) - sepwd }.
