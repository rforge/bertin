...
@
<<fig=TRUE, echo=FALSE, width=6, height=7, label=showCols>>=
...
@
<<fig=TRUE, echo=FALSE, width=6, height=7, label=showPals>>=
##------ Some palettes ------------
demo.pal <-
...
If your have to create your own palette, the basic tool is interpolation. It depends on yourview of the colour space. Here is linear interpolation in RGB space, with some variants.

<<fig=TRUE, label=RGBinter>>=
...
par(oldpar)
@
<<fig=TRUE, lable=colorRampPvar20>>=
oldpar <- par(mfrow=c(4,1), mar=c(2, 1, 2, 1) + 0.1)
mycolors <- c("red", "magenta", "blue")
...
@
\clearpage
\subsubsection{Betin Colour Palettes}
