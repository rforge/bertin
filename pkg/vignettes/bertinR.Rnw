...
%:cart and patches
%We will discuss an implementation of Bertin's ideas. Implemanting his examples as is, is trivial. To catch his ideas demands some thoughts, and has immediate consequences for an \R{} environment.
%
...
sampleimagem <- function(z, 
	...
	par(yaxt="n", fig=c(0, 1, 0, 0.2), 
		mar=c(3.5,4.0,0.5,4.0), new=TRUE)
...
The total user space has the size \ircode{(nrrows * (1+2*sepwd),} \ircode{nrcols * (1+2*sepwd)}. The drwing area for cell \ircode{x[i, j]} is a unit square with bootom left coordinates \ircode{ (nrrows-i+1)*}  \ircode{(1+2*sepwd) -} \ircode{sepwd*} \ircode{(1+2*sepwd)} \ircode{- sepwd,} \ircode{j* (1+2*sepwd) - sepwd }.
...
Bstep <- c(Bmone[1:floor(BMExplCols/2)], Bone[(1+floor(BMExplCols/2)):BMExplCols])
Bhat <- Bone
