...
# ok
bertinrect1 <- function(x,main = deparse(substitute(x)),
	sepwd=0.05,
	mar= c(1,1,6,4)+0.1,
	...){
	# [i,j] bottom left is at user coordinates (i,j)
...
bertinrect3 <- function(x,main = deparse(substitute(x)),
	...
	#parasp(x)
parasp(t(x))
	plot(c(1, (ncol(x)+1)*(1+2*sepwd)), c(1, (nrow(x)+1)*(1+2*sepwd)), 
	main=main,type= "n", xlab="", ylab="", axes=FALSE,...)
print(par("pin"))
	x <- as.matrix(x) #! support lists and data frames as well
	...
	xleft <- matrix((1:ncol(x)), nrow(x), ncol(x), byrow=TRUE)*(1+2*sepwd)
	xright <- xleft+1
	xbottom <-  (1+nrow(x)-(matrix((1:nrow(x)),nrow(x),ncol(x))))*(1+2*sepwd) 
		+zeroline #box zero line
	xtop <- x*scale+xbottom
...
if (any(ranges[2,]==0)){
	...
	#textnames(x)
       if (!is.null(colnames(x))){ 
       	for (col in (1:dim(x)[2])) 
       		text(col*(1+2*sepwd)+0.8, (nrow(x)+1)*(1+2*sepwd), colnames(x)[col], 
            pos = 3, xpd = NA, offs = 1, srt = 90, cex=0.6)}
       if (!is.null(rownames(x))) {
       	r <- par("usr")[2] #right
       	for (row in (1:dim(x)[1])) 
       		text(r, (nrow(x)-row) * (1+2*sepwd)+1.4, rownames(x)[row], 
            pos = 4, xpd = NA, offs = 0, srt = 0, cex=0.6)
            
   if (!all(is.finite(x))) {
   	badpos <- !is.finite(x)
   	xbottom <-  ((1+nrow(x))-(matrix((1:nrow(x)),nrow(x),ncol(x))))*(1+2*sepwd) #box zero line
   	text(xleft[badpos]+0.4,xbottom[badpos],labels=x[badpos], pos=3, offs=0.5,col="red",cex=0.6)
   }
} 
}
